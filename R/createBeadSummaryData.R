...
#library(limma)
  ...
  for(i in 1:length(arrays)){
    ...
    for(j in 1:length(probes)){
      R[j,i] = mean(BLData$R[o[,j]$valid,arrays[i]],na.rm=TRUE)

      if(!is.null(BLData$G)){
        G[j,i] = mean(BLData$G[o[,j]$valid,arrays[i]],na.rm=TRUE)
      }
   
      Rb[j,i] = mean(BLData$Rb[o[,j]$valid,arrays[i]],na.rm=TRUE)

      if(!is.null(BLData$Gb)){
        Gb[j,i] = mean(BLData$Gb[o[,j]$valid,arrays[i]],na.rm=TRUE)
      }
      
      beadstdev[j,i]  = sd(BLData$R[o[,j]$valid,arrays[i]], na.rm=TRUE)

      nobeads[j,i] = length(o[,j]$valid)

      nooutliers[j,i] = length(o[,j]$outliers)
    }
  }
