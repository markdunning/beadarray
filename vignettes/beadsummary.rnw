...
boxplot(exampleSummaryData.log2[ids,], 
  sampleFactor = "SampleFac", probeFactor = "IlluminaID")
@

\subsection{A note about ggplot2}
The \Rfunction{boxplot} function in \Rpackage{beadarray} creates graphics using the \Rpackage{ggplot2} package rather than the {\tt R} base graphics system. Therefore, the standard way of manipulating graphics using {\tt par} and {\tt mfrow} etc will not work with the output of \Rfunction{boxplot}. However, the \Rpackage{ggplot2} package has equivalent functionality and is a more powerful and flexible system. There are numerous tutorials on how to use the \Rpackage{ggplot2} package, which is beyond the scope of this vignette. In the below code, we assign the results of \Rfunction{boxplot} to objects that we combine using the \Rpackage{gridExtra} package. The code also demonstrates how aspects of the plot can be altered programatically. 
...
Replicate samples can also be compared using the \Rfunction{maplots} function. 

<<MAs>>=

mas <- maplots(exampleSummaryData[,1:12],do.log=FALSE)
...
probeFactor = "PROBEQUALITY", sampleFactor="SampleFac") + theme(axis.text.x=element_text(angle=45, hjust=1)) +  xlab("Probe Quality") + ylab("Log2 Intensity")
...
<<>>=
limmaRes <- limmaDE(exampleSummaryData.filt, SampleGroup=rna)
limmaRes
...
To assist with meta-analysis, and integration with other genomic data-types, it is possible to export the normalised values as a \Rclass{GRanges} object. Therefore it is easy to perform overlaps, counts etc with other data using the \Biocpkg{GenomicRanges} and \Biocpkg{GenomicFeatures} packages. In order for the ranges to be constructed, the genomic locations of each probe have to be obtained from the appropriate annotation package (\Rpackage{illuminaHumanv3.db} in this example). Provided that this package has been installed, the mapping should occur automatically.

<<>>=
gr <- as(exampleSummaryData.filt, "GRanges")
@

The limma analysis results can also be exported as a GRanges object for downstream analysis. The element metadata of the output object is set to the statistics from the limma analysis.
