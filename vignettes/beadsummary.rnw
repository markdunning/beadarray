...
\subsection{Readings IDAT files}

<<readIDAT, eval=FALSE>>=
downloadDir <- tempdir()
getGEOSuppFiles("GSE27073", baseDir = downloadDir)
idatFiles <- list.files(path = "~/Data/GEO_idat/", pattern = ".idat", full.names=TRUE)
gunzip(idatFiles)
idatFiles <- list.files(path = "~/Data/GEO_idat/", pattern = ".idat", full.names=TRUE)
BSData <- readIdatFiles(idatFiles[1])
@
