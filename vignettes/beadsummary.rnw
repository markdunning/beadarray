...
To make the most use of the code in this vignette, you will need to install the \Biocexptpkg{beadarrayExampleData} and \Biocannopkg{illuminaHumanv3.db} packages from {\tt Bioconductor}. 
...
mas <- maplots(exampleSummaryData.log2,do.log=FALSE)
...
<<>>=
mas <- maplots(exampleSummaryData.log2,do.log=FALSE,SampleGroup="SampleFac")
mas[[1]]
@
...
Alternatively, GEO's official probe annotation files can be used to decide which probe to include in the submission. You will first have to download the appropriate file from the GEO website.
