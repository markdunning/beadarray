...
each array.
The summarised intensities (\emph{bead-summary data}) output by Illumina's BeadStudio software can 
also be imported using the \Rpackage{beadarray} package.

The documentation for this package is split into separate user guides: one for bead-level data, 
a second for bead-summary data, and a third which provides details and examples of using the 
BASH (\textbf{b}ead\textbf{a}rray \textbf{S}ubversion of \textbf{H}arshlight) method for detecting 
spatial artefacts. 
...
Cairns JM, Dunning MJ, Ritchie ME, Russell R, Lynch AG. \emph{BASH: A tool for managing BeadArray spatial artefacts}. (submitted).\newline
