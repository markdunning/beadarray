...
\SweaveOpts{eval=FALSE, keep.source=FALSE}
...
This vignette demonstrates the processing of bead-level data using beadarray. The data used are a subset of the data presented in the \Rpackage{BeadArrayUseCases} package and can be downloaded in the \Rpackage{beadarrayExampleData} package. A more comprehensive commentary on the analysis of Illumina BeadArray package is given in the vignette of \Rpackage{BeadArrayUseCases}, including other analysis tools that are not part of \Rpackage{beadarray}. 
...
The raw images and text files required to perform a bead-level analysis are produced by Illumina's BeadScan software. 
...
The command to read bead-level data from the current working directory is as follows. Note however that no raw data are included in the \Rpackage{beadarrayExampleData} so this command cannot be executed. See the \Rpackage{BeadArrayUseCases} package for some example data to try out this function.
...
Note that it is not compulsory to specify which type of Illumina assay was used to generate the data. However, for expression data it is common to specify the name of the platform using one of the strings Humanv4, Humanv3, Humanv2, Humanv1, Mousev2, Mousev1p1, Mousev1 or Ratv1. This information can be used to identify control probes and perform probe mapping. The \Rfunction{suggestAnnotation} function may be used if you are unsure of which string to use. This checks the overlap between the bead IDs found in the data with a collection of IDs extracted from Illumina's annotation files. For the example data stored with the \Robject{beadarrayExampleData}, the suggested annotation is {\tt Humanv3}. Provided the \Rpackage{illuminaHumanv3.db} package is present, \Rpackage{beadarray} will be able to annotate \Robject{beadarrayExample}.
...
<<Imageplots, fig=TRUE, echo=TRUE, width=6, height=2>>=
...
The result of bash includes quality control scores; the number of beads masked in total and the extended score. We have already saved the quality control into the \Robject{exampleBLdata} object and they can be retrieved in the following way

<<getBASH,echo=TRUE>>=

exampleBLdata@sectionData$BashQC


@

The weights themselves can be stored using \Rfunction{setWeights}. 
...
<<masks,echo=TRUE,fig=TRUE, width=6, height=2>>=
par(mar = c(2,2,2,2))
...
<<positiveControls, echo=TRUE,fig=TRUE, width=6, height=4>>=

par(mfrow=c(1,2), mar = c(2,2,2,2))

for(i in 1:2){
	poscontPlot(exampleBLdata, array=i,ylim = c(10,16))
}
@
