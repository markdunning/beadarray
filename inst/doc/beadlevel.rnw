...
the \emph{background} pixel intensities surrounding each bead. It should not to be confused with another background correction further along the analysis pipeline, which may involve negative control beads to account for non-specific binding.
...
<<BeadLevelBoxplots, fig=TRUE,echo=TRUE>>=
par(mfrow=c(1,2))
...
<<bashResults, echo=TRUE, eval=FALSE>>=
...
controlProfile <- makeControlProfile("Humanv3")
...
<<positiveControls, echo=TRUE,fig=TRUE>>=
...
<<outlierPlots, echo=TRUE, fig=TRUE, width=6, height=2>>=

par(mfrow=c(1,2), mar = c(2,2,2,2))

outlierplot(exampleBLdata, array=1)

outlierplot(exampleBLdata, array=2)
...
greenChannel2 <- new("illuminaChannel", logGreenChannelTransform, illuminaOutlierMethod, myMedian, myMad,"G")

BSData2 <- summarize(exampleBLdata, list(greenChannel2))

@

<<showBSData, echo=TRUE, esults=hide>>=
BSData
