...
<<normExp, echo=FALSE, eval=FALSE>>==

normExpPars = matrix(nrow=10, ncol=3)
...
##Estimate parameters from normexp
 ...
 res = limma:::normexp.fit.control(x=fooMat, status = beadStatusVector(BLData, array=i,controlProfile=controlProfile), negctrl = "negative")
...
for(i in 1:10){
	corSignal = limma:::normexp.signal(x = getBeadData(BLData, array=i, what="Grn"), par = normExpPars[i,])
...
<<showBSData, echo=TRUE, eval=TRUE>>==
BSData
...
<multipleChannel,echo=TRUE,eval=TRUE>>==
