...
This vignette demonstrates the analysis of bead summary data using beadarray. The data used are a summarized version of the bead-level data presented in the \Rpackage{BeadArrayUseCases} package and can be downloaded in the \Rpackage{beadarrayExampleData} package. A more comprehensive commentary on the analysis of Illumina BeadArray package is given in the vignette of \Rpackage{BeadArrayUseCases}, including other analysis tools that are not part of \Rpackage{beadarray}. 
...
exprs(exampleSummaryData)[1:10,1:2]
se.exprs(exampleSummaryData)[1:10, 1:2]

@


The \Robject{exampleSummaryData} object was created by following the steps described in the {\tt beadlevel.pdf} vignette, but for the entire bead-level MAQC dataset described in the \Rpackage{BeadArrayUseCases} package. Summarized data are stored in an object of type \Rclass{ExpressionSetIllumina} which
...
@
Boxplots of intensity levels and the number of beads are useful for quality assessment purposes. Below
is the code to produce boxplots of these quantities for each array in the experiment.


<<boxplot, fig=TRUE, echo=TRUE,width=6, height=4>>=

par(mfrow=c(1,2), mar = c(2,2,2,2))
boxplot(exprs(exampleSummaryData),outline=FALSE)
boxplot(nObservations(exampleSummaryData),outline=FALSE)

@

Replicate observations can also be compared using the \Rfunction{plotMAXY}

<<MAs, fig=TRUE, echo=TRUE,width=6, height=6>>=
...
exampleSummaryData.q <- normaliseIllumina(exampleSummaryData, method="quantile", transform="none")

@


\section{Differential expression and filtering}
...
ids <- as.character(featureNames(exampleSummaryData.q))
...
exampleSummaryData.filt <- exampleSummaryData[!rem,]
...
<<deanalysis, eval=TRUE,fig=TRUE,width=6, height=6>>=
library(limma)
